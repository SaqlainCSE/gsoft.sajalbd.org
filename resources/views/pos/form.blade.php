<div class="row">
    <div class="col-12 col-md-12">
        <div class="mb-3 row">
            <div class="col-md-4">
                {{ Form::label('sales_type', __('Sales Type'), ['class' => 'col-form-label text-start']) }}
                {{ Form::select('sale_type_id', $sales_types, $order->sale_type_id, ['required' => true, 'id' => 'sale_type_id', 'class' => 'form-control' . ($errors->has('sale_type_id') ? ' is-invalid' : ''), 'placeholder' => 'Select Sales Type']) }}
                {!! $errors->first('sale_type_id', '<div class="invalid-feedback">:message</div>') !!}
            </div>
            ...
            <div class="col-md-4">
                ...
                @php
                    $clients = [];
                    if ($order->client) {
                        $clients[$order->client->id] = $order->client->name;
                    }
                @endphp

                {{ Form::select('client', $clients, $order->client ? $order->client->id : '', ['required' => true, 'id' => 'client', 'class' => 'form-control' . ($errors->has('client') ? ' is-invalid' : ''), 'placeholder' => 'Select client']) }}
                {!! $errors->first('client', '<div class="invalid-feedback">:message</div>') !!}
            </div>
            <div class="col-md-4">
                {{ Form::label('sell_by', __('Sell By'), ['class' => 'col-form-label text-start']) }}
                {{ Form::select('sell_by', $users, $order->sell_by, ['required' => true, 'id' => 'sell_by', 'class' => 'form-control' . ($errors->has('sell_by') ? ' is-invalid' : ''), 'placeholder' => 'Select Sell By', 'required' => true]) }}
            ...
            <div class="col-md-12 mt-4">
                <table class="table table-bordered nowrap"
                    ...
                    <tbody id="selected_product_list">
                        ...
                        @if ($order->meta)
                            @foreach ($order->meta as $key => $meta)
                                <tr id="row_id_{{ $key + 1 }}">
                                    <td>
                    ...
                    <tfoot>
                        ...
                        <tr>
                            ...
                            <td id="return_amount" style="padding: 1px;" colspan="2">
                                {{ Form::text('return_amount', $order->return_amount ?: 0, ['id' => 'returnAmount', 'required' => false, 'class' => 'form-control', 'required' => true]) }}
                            </td>
                        </tr>
                        <tr>
                            <td class="text-end" colspan="7">
...
@push('js')
    <script>
        ...
        function removeRow(id) {
            ...
            selectedProduct = selectedProduct.filter(function(e) { return e != elem })
            $("#row_id_" + id).remove();
        ...
        function calculateUnitPrice(row) {
            ...
            //var wage = $("#" + row + " .wage").val();
            console.log(wage)
            var subtotal = (parseFloat(weight) * parseFloat(unitprice)) + parseFloat(wage ? wage : 0) + parseFloat(stDiaPrice ? stDiaPrice : 0);           
        ...
        $("#preview").on("click", function(e){
            ...
            openWindow("{{ route('order-preview') }}?" + formData);
        });
